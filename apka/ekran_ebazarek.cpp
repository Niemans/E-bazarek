...
void __fastcall TForma_ekran_bazarek::GridCellClick(TColumn * const Column, const int Row)

{
	int id = Grid->Cells[grid_colId->Index][Row].ToInt();
	TForm_ekran_przedmiotu * ekran_przedmiotu = new TForm_ekran_przedmiotu(this, ADOConnection, id);
	ekran_przedmiotu -> ShowModal();
    delete ekran_przedmiotu;
}
//---------------------------------------------------------------------------
